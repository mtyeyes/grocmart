...
const MasonryGallery = () => {
  ...
  const getLoadedData = (requestResults: {
    'gallery-images': GalleryStateItem[];
  }) => {
    setGalleryState(requestResults['gallery-images']);
  };
  ...
  useEffect(() => {
    ...
    if (galleryState.length !== 0) {
      galleryState.forEach((item) =>
        filterGroups.add(productsState[item.productId].group),
      );
      setFilterState((prevState) => {
        return { ...prevState, availableFilters: ['all', ...filterGroups] };
      });
    }
  ...
  useEffect(() => {
    if (filterState.selectedFilter === 'all') {
      ...
      const filteredItems = galleryState.filter(
        (item) =>
          productsState[item.productId].group === filterState.selectedFilter,
      );
      setItemsToDisplay(filteredItems);
    }
  }, [filterState, galleryState, productsState]);

  const galleryMapCallback = ({
    imageName,
    imageDescription,
    productId,
  }: GalleryStateItem) => {
    const productFinalPrice = countPriceAfterDiscounts(
      productId,
  ...
  return (
    <section className="gallery">
      <Loader
        requests={{
          resourceRequests: ['gallery-images'],
