...
const Header = () => {
  ...
  const floatingNumberOfItems = (
    <FloatingText positionStyle={{ top: -5, right: -12, width: 20 }} valueToTriggerAnimation={itemsInCart}>
      {itemsInCart <= 9 ? itemsInCart : '9+'}
    </FloatingText>
  );
      ...
      <header className={isSticked ? 'header header--stick' : 'header'}>
        <div className="header__wrapper">
          ...
          <ModalToggler
            ...
            childrenBlockName="search"
            icon="search"
            render={(switchModalVisibility) => <Search switchModalVisibility={switchModalVisibility} />}
          />
          <ModalToggler
            parentBlockName="header"
            ...
            render={(switchModalVisibility) => <ModalCart switchModalVisibility={switchModalVisibility} />}
          />
        </div>
      </header>
