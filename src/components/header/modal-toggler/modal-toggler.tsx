...
const ModalToggler = ({
  ...
  return (
    <div
      className={`modal-toggler ${parentBlockName}__modal-toggler ${parentBlockName}__modal-toggler--${childrenBlockName} ${
        isModalVisible ? `${parentBlockName}__modal-toggler-toggled` : ''
      }`}
    >
      <Button className="modal-toggler__btn" onClick={switchModalVisibility}>
        <Icon iconId={icon} />
        <HiddenText>{childrenBlockName}</HiddenText>
        {btnChildrenElement !== undefined && btnChildrenElement}
      </Button>
      {isModalVisible && render(switchModalVisibility)}
    </div>
  );
};
